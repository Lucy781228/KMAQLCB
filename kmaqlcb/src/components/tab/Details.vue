...
export default {
    ...
    methods: {
        ...
        async updateUser() {
            try {
                console.log(this.user)
                const response = await axios.put('/apps/kmaqlcb/update_user', {
        ...
        async getUser() {
            try {
                const response = await axios.get(generateUrl(`/apps/kmaqlcb/user/${this.userId}`));
