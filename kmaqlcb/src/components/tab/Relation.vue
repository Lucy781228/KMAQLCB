...
export default {
    ...
    methods: {
        ...
        async getRelations() {
            try {
                const response = await axios.get(generateUrl(`/apps/kmaqlcb/relations/${this.userId}`));
        ...
        async onDeleteRelations() {
            try {
                const deletePromises = this.selectedRelations.map(relation =>
                    axios.delete(generateUrl(`apps/kmaqlcb/delete_relation/${relation.relation_id}`))
                );
