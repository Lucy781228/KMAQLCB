...
export default {
    ...
    methods: {
        ...
        async getEducations() {
            try {
                const response = await axios.get(generateUrl(`/apps/kmaqlcb/educations/${this.userId}`));
        ...
        async onDeleteEducations() {
            try {
                const deletePromises = this.selectedEducations.map(education =>
                    axios.delete(generateUrl(`apps/kmaqlcb/delete_education/${education.education_id}`))
                );
