...
export default {
    ...
    methods: {
        ...
        async getUsers() {
            try {
                const response = await axios.get(generateUrl('apps/kmaqlcb/all_users'))
        ...
        async onDeleteUsers() {
            try {
                const deletePromises = this.selectedUsers.map(user =>
                    axios.delete(generateUrl(`apps/kmaqlcb/delete_user/${user.qlcb_uid}`))
                );
        ...
        async getUsersByType() {
            try {
                if (this.types.length > 0) {
                    ...
                    const response = await axios.get(generateUrl('/apps/kmaqlcb/filter'), { params });
